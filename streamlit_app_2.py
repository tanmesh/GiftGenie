...
class GiftSuggestionWorkflow(Workflow):
    ...
    async def tweet_analyzer(
        self, ctx: Context, ev: TweetAnalyzerEvent
    ) -> InterestMapperEvent:
        if "tweet_analyzer_agent" not in ctx.data:
            ...
            ctx.data["tweet_analyzer_agent"] = create_agent(
                ctx, [categorize_tweets], system_prompt
            )

        interests = ctx.data["tweet_analyzer_agent"].chat(
            f"Analyze these tweets: {ev.tweets}"
        )
        return InterestMapperEvent(interests=str(interests))

    @step(pass_context=True)
    async def interest_mapper(
        self, ctx: Context, ev: InterestMapperEvent
    ) -> GiftIdeaGeneratorEvent:
        if "interest_mapper_agent" not in ctx.data:
            ...
            ctx.data["interest_mapper_agent"] = create_agent(
                ctx, [map_interests_to_gift_categories], system_prompt
            )

        gift_categories = ctx.data["interest_mapper_agent"].chat(
            f"Map these interests to gift categories: {ev.interests}"
        )
        return GiftIdeaGeneratorEvent(gift_categories=str(gift_categories))

    @step(pass_context=True)
    async def gift_idea_generator(
        self, ctx: Context, ev: GiftIdeaGeneratorEvent
    ) -> GiftDebaterEvent:
        if "gift_idea_generator_agent" not in ctx.data:
            ...
            ctx.data["gift_idea_generator_agent"] = create_agent(
                ctx, [generate_affordable_gift_ideas], system_prompt
            )

        gift_ideas = ctx.data["gift_idea_generator_agent"].chat(
            f"Generate gift ideas for these categories: {ev.gift_categories}"
        )
        return GiftDebaterEvent(gift_ideas=str(gift_ideas))

    @step(pass_context=True)
    async def gift_debater(
        self, ctx: Context, ev: GiftDebaterEvent
    ) -> GiftReasonerEvent:
        if "gift_debater_agent" not in ctx.data:
            ...
            ctx.data["gift_debater_agent"] = create_agent(
                ctx, [debate_gift_ideas], system_prompt
            )

        debates = ctx.data["gift_debater_agent"].chat(
            f"Debate these gift ideas: {ev.gift_ideas}"
        )
        return GiftReasonerEvent(debates=str(debates))

    @step(pass_context=True)
    async def gift_reasoner(
        self, ctx: Context, ev: GiftReasonerEvent
        ...
        if "gift_reasoner_agent" not in ctx.data:
            ...
            ctx.data["gift_reasoner_agent"] = create_agent(
                ctx, [reason_over_debates], system_prompt
            )

        final_gifts = ctx.data["gift_reasoner_agent"].chat(
            f"Reason over these debates: {ev.debates}"
        )
        # Extract the content from the AgentChatResponse
        gift_ideas = str(final_gifts).strip().split("\n")
        return AmazonKeywordGeneratorEvent(gift_ideas=gift_ideas)

    @step(pass_context=True)
    async def amazon_keyword_generator(
        self, ctx: Context, ev: AmazonKeywordGeneratorEvent
    ) -> StopEvent:
...
async def run_workflow():
    ...
    with st.spinner("Generating gift ideas..."):
        gift_ideas_event = await workflow.gift_idea_generator(
            ctx, gift_categories_event
        )
